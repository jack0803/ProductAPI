...
const create = async data => {
    ...
    const query = `INSERT INTO ${tableName} (sku , name , status , created_at , updated_at) VALUES (? , ? , ? , ? , ?)`;
	console.log(data.name);

    const params = [data.sku , data.name , data.status, data.created_at , data.updated_at];
...
const list = async (page, limit , info) => {
	...
	if (page) {
		...
		const possiblePage = Math.ceil(pageConfig.CATEGORIES/limit);
		const offset = (page-1)*limit;
		limitString = `LIMIT ${limit} OFFSET ${offset}`;
		console.log(pageConfig.CATEGORIES);
	}

	const query = `SELECT sku,name,status,created_at,updated_at from ${tableName} where status !=? ${limitString} `;
...
const viewByCategoryId = async id => {
	...
	const query = `SELECT sku,name,status,created_at,updated_at FROM ${tableName} WHERE sku=?`;
	const params = [id, dataStatusText.DELETED];
	const qData = await dbConnection.query(query, params);
	return qData[0] || null;
};

const viewByCategoryName = async id => {
	console.log(id);
	// const query = `SELECT first_name, last_name, email, type, photo, status, created_at, updated_at FROM ${tableName} WHERE user_id=? and status!=?`;
	const query = `SELECT sku,name,status,created_at,updated_at FROM ${tableName} WHERE name=?`;
	const params = [id, dataStatusText.DELETED];
	const qData = await dbConnection.query(query, params);
	return qData[0] || null;
};
...
const update = async (id, data) => {
	const query = `UPDATE ${tableName} SET sku=?,name=?,status=?,updated_at=? where sku=?`;

	const params = [data.sku, data.name, data.status, data.updated_at, id];
